#![forbid(unsafe_code)]
//! Serving the four avenues from `gha-indie-worker-api-server`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `gha-indie-worker-lib-core` |
...
///
/// Bind `GHA_INDIE_WORKER_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `GHA_INDIE_WORKER_NATS_URL`.
///
/// # Errors
